  ...
  return (
    ...
    <div className={`w-full min-h-screen font-sans relative ${theme === 'light' ? 'bg-white' : 'bg-black'}`}>
          ...
          <img
            src={mclaren}
            alt="bg"
            className=" fixed absolute inset-0 w-full h-full object-cover z-0 opacity-10"
          />
    <div className="bg-black min-h-screen flex flex-col pt-32 pb-16">
              <p className= "mt-13 text-center  text-[#94d2bd] text-sm  tracking-[0.5em] uppercase font-bold mb-4">— Assistance 24X7</p>

      <div className="text-center px-4 max-w-3xl mx-auto mb-12">
        <h1 className="mt-15 text-4xl md:text-5xl font-extrabold text-white mb-4 tracking-tight">
          Emergency <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#bfe8dd] to-[#94d2bd]"> Roadside Assistance</span>
        </h1>
        <p className="text-lg text-white">
          Stuck on the road? Don't worry, RESQUERIDE is here to help. Select a service below, and our nearest professional will be dispatched to your location immediately.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {services.map((service) => (
            <div
              ...
              className={`backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-6 text-center hover:border-[#94d2bd]/40 hover:bg-white/8 transition-all duration-300 cursor-pointer transition-all duration-300 transform hover:-translate-y-2 rounded-2xl p-6 flex flex-col items-center text-center shadow-md border-2 ${
                selectedService === service.id
                  ? "border-[#94d2bd] bg-[#94d2bd]/10 shadow-[#94d2bd]/30"
                  : "border-transparent bg-white hover:shadow-xl hover:border-gray-200"
              }`}
            >
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors ${
                  selectedService === service.id ? "bg-[#94d2bd] text-white" : "bg-[#94d2bd] text-white"
                }`}
              >
                {service.icon}
              </div>
          ...
          {submitted ? (
            <div className="flex flex-col items-center text-center py-10">
              ...
              <p className="text-gray-600 mb-8 max-w-sm">
                Our support team has received your request. A driver will contact you shortly and head to your location.
              </p>
              <button
                onClick={() => {
                  setSubmitted(false);
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="text-white/40 text-xs uppercase text-left tracking-widest block  font-medium  mb-1">Service Required</label>
                  <div className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-white/20 outline-none focus:border-[#94d2bd]/60 transition-all">
                    {selectedService ? services.find((s) => s.id === selectedService).name : "Select a service from above"}
                  </div>
                </div>

                <div>
                  <label htmlFor="location" className="text-white/40 text-xs uppercase text-left tracking-widest block  font-medium  mb-1">Current Location (Landmark/Address)</label>
                  <input
                    type="text"
                    id="location"
                    name="location"
                ...
                <div>
                  <label htmlFor="vehicle" className="text-white/40 text-xs uppercase text-left tracking-widest block  font-medium  mb-1">Vehicle Details (Make, Model, License Plate)</label>
                  <input
                    type="text"
                    id="vehicle"
                    name="vehicle"
                ...
                <div>
                  <label htmlFor="description" className="text-white/40 text-xs uppercase text-left tracking-widest block  font-medium  mb-1">Issue Description (Optional)</label>
                  <textarea
                    id="description"
                    name="description"
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    className="w-full bg-[#94d2bd] hover:bg-[#7bbda8] text-white font-bold text-lg py-4 rounded-xl flex justify-center items-center gap-2 transform transition hover:scale-[1.02] shadow-lg shadow-[#94d2bd]/30"
                  >
                    <PhoneCall size={20} />
